...
def inicializaRAID ():
    ...
    nome_disco = input('Digite o nome do disco: ')

    num_discos = int(input('Digite a quantidade de discos: '))
    if num_discos >= 3:
        ...
        if tam_bloco  > tam_discos:    
            ...
            print('Não é possivel ter blocos maiores que o disco inteiro.')        
    else:
...
def obtemRAID ():
    ...
    if 'discos' in dados.keys() and 'pasta' in dados.keys():
        ...
        exibir_pasta = input('Em que pasta você armazenou os discos? ')
        if exibir_disco in dados.get('discos') and exibir_pasta == dados.get('pasta'):
            print('Os arquivos e a pasta existem.')
        ...
        print('Você não inicializou o RAID')
                ...
                if bloco_escrever in b:
                    ...
                    for c in range(0,len(dados.get(dados.get(a)))):
                        if b[c] == bloco_escrever:
                            ...
                            break
####
        escrever = int(input('Quantos bytes você deseja ler? '))
        ...
        print('O RAID ainda não foi iniciado.')        
                ...
                if bloco_ler in b:
                    ...
                    for c in range(0,len(dados.get(dados.get(a)))):
                        if b[c] == bloco_ler:
                            ...
                            break
####
        ler = input('Digite o que você deseja gravar do disco: ')
        dados_ler = len(ler)
        cont = 1
...
def removeDiscoRAID ():
    ...
    try:
        ...
        print('Não foi possivel apagar o arquivo.')

def constroiDiscoRAID ():
    if dados.get('disco_apagado'):
        xors = []
        f = open(f'home/dorbado/Documentos/Faculdade/computaria/{dados.get('pasta')}/{dados.get('arquivo_apagado')}','wb')
        f.close()
        for a in range(0,dados.get('quant_blocos')):
            xors = []
            for b in dados.get('discos'):
                if b != dados.get('arquivo_apagado'):
                    f = open(f'home/dorbado/Documentos/Faculdade/computaria/{dados.get('pasta')}/{b}','wb') #wr?
