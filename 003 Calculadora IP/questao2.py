...
ip2 = input ("Digite a máscara de rede (em bits): ")
'''
...
try:
    ip_source =  input("Digite o endereço IP no seguinte formato: 192.168.10.5\nEndereço IP: ") #'10.14.11.10' #
    if not(ip_source.count(".")==3 and len(ip_source)>=7):
        net_mask = 34
        print("IP Inválido!")
except: print("IP Inválido!")

if (ip_source.count(".")==3 and len(ip_source)>=7):
    try:
        net_mask = int(input("Digite a mascara da rede no seguinte formato: 24\nMascara da rede: ")) #16 #
    except: 
        net_mask = 34
        print("Mascara inválida!")

if net_mask <33 and ip_source.count(".")==3 and len(ip_source)>=7:
