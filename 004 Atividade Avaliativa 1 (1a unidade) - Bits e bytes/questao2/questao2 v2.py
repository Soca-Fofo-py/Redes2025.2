...
def inicializaRAID ():
    ...
    if num_discos >= 3:
        ...
        if tam_bloco  > tam_discos:    
            ...
            for a in range(1,quant_blocos):
                
                dados = {}

                for b in discos: 
                    if b != discos[len(discos) - 1]:
                        f = open(f'/home/dorbado/Documentos/Faculdade/computaria/{pasta}/{b}','rb')
                        f.seek(tam_bloco*(fileira))
                        dados[a+'_fileira_'+str(fileira)] = f.read(tam_bloco) 
                        f.close()
                
                xors['fileira_'+str(fileira)] = dados.values()

                fileira += 1
           ##### pode se tornar uma função #####
            for b in xors:    
                for a in range(1,len(b)):
                    if a != 1:        
                        ...
                        xor = b[0] ^ b[a]
            ##### pode se tornar uma função #####
                f = open(f'/home/dorbado/Documentos/Faculdade/computaria/{pasta}/{discos[len(discos) - 1]}','rb')
                f.write(xor)
