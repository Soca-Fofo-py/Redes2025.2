...
if len(sys.argv) == 2:
    try:
        Host , Port = sys.argv[1].split(":") # split pode gerar erros EX: 192.: - IP = "192." e porta = ""
...
print(f"Conectando-se ao Host: {Host}, na Porta: {Port}")
####################################################################################################
...
if operaçao == 30: ####################################################################################################
    ...
    if os.path.isfile(var1) == True: # Se arquivo existir:
        ...
        tam_nome_c = tam_nome_c.to_bytes(4,Endianess)
        ...
        status = int.from_bytes(status,Endianess)
        if status == statusPositivo:
            print("Tentativa de upload aceito pelo servidor.")
            ...
            status_retorno = tcp_socket.recv(1)
            status_retorno = int.from_bytes(status_retorno,Endianess)
            if status_retorno == statusPositivo:
                print("Upload bem sucedido!")
            else: print("Erro Inesperado!!!")
        else:
            print("Arquivo não aceito pelo servidor!!!")
    else: 
        #tcp_socket.send(b_statusErro)
        #tcp_socket.send(((0).to_bytes(4,Endianess)))
        print("Arquivo NÃO encontrado!")
